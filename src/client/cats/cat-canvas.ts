...
export const
  CatCanvas = (cat: CatFM, img: HTMLImageElement) => canvas(
    ...
    withCtx(
      (ctx) => {
        ...
        const
          { data, width, height } = ctx.getImageData(0, 0, ctx.canvas.width, ctx.canvas.height),
        ...
        for (let y = 0; y < height; ++y) {
          for (let x = 0; x < width; ++x) {
            if (cmp(x, y, 0, 0, 0) || (y >= 16 && cmp(x, y, 255, 255, 0))) {
